...
public interface StatusRepositoryV2 extends JpaRepository<StatusV2,Integer> {
    ...
    @Query("SELECT DISTINCT s FROM StatusV2 s JOIN s.tasks t JOIN t.board b WHERE b.id = :boardId AND s.is_fixed_status = false GROUP BY s HAVING COUNT(t) > :taskLimit")
